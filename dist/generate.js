...
import { buildEditPrompt, buildUserPrompt, EDIT_SYSTEM_PROMPT, schemaDescription, SYSTEM_PROMPT, } from "./prompt.js";
import { getProvider } from "./providers/index.js";
import { tripInputSchema } from "./schema/input.js";
import { tripPlansJsonSchema } from "./schema/json-schema.js";
import { itineraryPlanSchema, tripPlansSchema, } from "./schema/output.js";
 ...
 */
async function generateValidPlans(provider, system, buildUser, extraCheck) {
    let repairHint;
    let lastError;
...
export async function generateItinerary(input) {
    ...
    const plans = await generateValidPlans(provider, SYSTEM_PROMPT, (repairHint) => buildUserPrompt({
        trip: parsed,
        planCount,
 ...
 */
export async function editItinerary(input) {
    const parsed = editItineraryInputSchema.parse(input);
    const { provider } = providerFromEnv();
    const description = schemaDescription();
    const dayCount = parsed.plan.days.length;
    const plans = await generateValidPlans(provider, EDIT_SYSTEM_PROMPT, (repairHint) => buildEditPrompt({
        plan: parsed.plan,
