...
import type { ItineraryPlan } from "./schema/output.js";
...
function tripRequest(trip: TripInputParsed): Record<string, unknown> {
  return {
...
function appendRepairHint(lines: string[], repairHint?: string): void {
  if (repairHint) {
    lines.push(
      "",
...
export function buildEditPrompt(input: {
  ...
  lines.push(
    `Current plan (lang: ${lang}):`,
    "",
    JSON.stringify(plan, null, 2),
    "",
    "Traveller's change request (between the markers):",
    "<<<",
    instruction,
    ">>>",
    "",
